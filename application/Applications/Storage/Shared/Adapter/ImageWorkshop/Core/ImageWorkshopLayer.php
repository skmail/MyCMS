 ...
 */
class App_Storage_Shared_Adapter_ImageWorkshop_Core_ImageWorkshopLayer
{
    // ===================================================================================
    // Properties
    // ===================================================================================
    ...
    public function __construct($image)
    {
        ...
        if (!extension_loaded('gd')) {
            throw new App_Storage_Shared_Adapter_ImageWorkshop_ImageWorkshopLayerException('PHPImageWorkshop requires the GD extension to be loaded.', static::ERROR_GD_NOT_INSTALLED);
        }
    	
        if (gettype($image) != 'resource' && gettype($image) != '\resource') {
            throw new App_Storage_Shared_Adapter_ImageWorkshop_ImageWorkshopLayerException('You must give a php image var to initialize a layer.', static::ERROR_PHP_IMAGE_VAR_NOT_USED);
        }
    ...
    public function mergeDown($layerId)
    {
        ...
        if ($this->isLayerInIndex($layerId)) {
            ...
            if ($layerLevel > 1) {
                ...
                $layerTmp = App_Storage_Shared_Adapter_ImageWorkshop_ImageWorkshop::initVirginLayer($layerTmpWidth, $layerTmpHeight);
                ...
                $layerTmp = App_Storage_Shared_Adapter_ImageWorkshop_ImageWorkshop::initFromResourceVar($this->image);
    ...
    public function crop($unit = self::UNIT_PIXEL, $width = 0, $height = 0, $positionX = 0, $positionY = 0, $position = 'LT')
    {
        ...
        if (($width != $this->width || $positionX == 0) || ($height != $this->height || $positionY == 0)) {
            ...
            $layerTmp = App_Storage_Shared_Adapter_ImageWorkshop_ImageWorkshop::initVirginLayer($width, $height);
            $layerClone = App_Storage_Shared_Adapter_ImageWorkshop_ImageWorkshop::initVirginLayer($this->width, $this->height);
    ...
    public function opacity($opacity, $recursive = true)
    {
        ...
        $transparentImage = App_Storage_Shared_Adapter_ImageWorkshop_Core_ImageWorkshopLib::generateImage($this->getWidth(), $this->getHeight());

        App_Storage_Shared_Adapter_ImageWorkshop_Core_ImageWorkshopLib::imageCopyMergeAlpha($transparentImage, $this->image, 0, 0, 0, 0, $this->getWidth(), $this->getHeight(), $opacity);
    ...
    public function flip($type = 'horizontal')
    {
        ...
        $temp = App_Storage_Shared_Adapter_ImageWorkshop_Core_ImageWorkshopLib::generateImage($this->width, $this->height);
    ...
    {
        $RGBTextColor = App_Storage_Shared_Adapter_ImageWorkshop_Core_ImageWorkshopLib::convertHexToRGB($color);
    ...
    public function write($text, $fontPath, $fontSize = 13, $color = 'ffffff', $positionX = 0, $positionY = 0, $fontRotation = 0)
    {
        if (!file_exists($fontPath)) {
            throw new App_Storage_Shared_Adapter_ImageWorkshop_ImageWorkshopLayerException('Can\'t find a font file at this path : "'.$fontPath.'".', static::ERROR_FONT_NOT_FOUND);
        }
        
        $RGBTextColor = App_Storage_Shared_Adapter_ImageWorkshop_Core_ImageWorkshopLib::convertHexToRGB($color);
        $textColor = App_Storage_Shared_Adapter_ImageWorkshop_imagecolorallocate($this->image, $RGBTextColor['R'], $RGBTextColor['G'], $RGBTextColor['B']);
    ...
    public function getResult($backgroundColor = null)
    {
        ...
        foreach ($this->layerLevels as $layerLevel => $layerId) {
            ...
            if ($this->layerPositions[$layerId]['x'] != 0 || $this->layerPositions[$layerId]['y'] != 0) {
                ...
                $virginLayoutImageTmp = App_Storage_Shared_Adapter_ImageWorkshop_Core_ImageWorkshopLib::generateImage($this->width, $this->height);
                App_Storage_Shared_Adapter_ImageWorkshop_Core_ImageWorkshopLib::mergeTwoImages($virginLayoutImageTmp, $imagesToMerge[$layerLevel], $this->layerPositions[$layerId]['x'], $this->layerPositions[$layerId]['y'], 0, 0);
        ...
        foreach ($imagesToMerge as $imageLevel => $image) {
            App_Storage_Shared_Adapter_ImageWorkshop_Core_ImageWorkshopLib::mergeTwoImages($mergedImage, $image);
            $iterator++;
        }
        ...
        $backgroundImage = App_Storage_Shared_Adapter_ImageWorkshop_Core_ImageWorkshopLib::generateImage($this->width, $this->height, $backgroundColor, $opacity);
        App_Storage_Shared_Adapter_ImageWorkshop_Core_ImageWorkshopLib::mergeTwoImages($backgroundImage, $mergedImage);
    ...
    {
        $virginImage = App_Storage_Shared_Adapter_ImageWorkshop_Core_ImageWorkshopLib::generateImage($this->getWidth(), $this->getHeight()); // New background image

        App_Storage_Shared_Adapter_ImageWorkshop_Core_ImageWorkshopLib::mergeTwoImages($virginImage, $this->image, 0, 0, 0, 0);
        unset($this->image);
    ...
    protected function indexLayer($layerLevel, $layer, $positionX = 0, $positionY = 0, $position)
    {
        ...
        $this->layerPositions[$layerId] = App_Storage_Shared_Adapter_ImageWorkshop_Core_ImageWorkshopLib::calculatePositions($this->getWidth(), $this->getHeight(), $layer->getWidth(), $layer->getHeight(), $positionX, $positionY, $position);
    ...
    public function resizeBackground($newWidth, $newHeight)
    {
        ...
        $virginLayoutImage = App_Storage_Shared_Adapter_ImageWorkshop_Core_ImageWorkshopLib::generateImage($this->width, $this->height);
    ...
    {
		throw new App_Storage_Shared_Adapter_ImageWorkshop_ImageWorkshopLayerException('Method resizeInPourcent() was renamed resizeInPercent(). Use resizeInPercent() instead.', static::METHOD_DEPRECATED);
    }
    ...
    {
        throw new App_Storage_Shared_Adapter_ImageWorkshop_ImageWorkshopLayerException('Method resizeByLargestSideInPourcent() was renamed resizeByLargestSideInPercent(). Use resizeByLargestSideInPercent() instead.', static::METHOD_DEPRECATED);
    }
    ...
    {
        throw new App_Storage_Shared_Adapter_ImageWorkshop_ImageWorkshopLayerException('Method resizeByNarrowSideInPourcent() was renamed resizeByNarrowSideInPercent(). Use resizeByNarrowSideInPercent() instead.', static::METHOD_DEPRECATED);
    }
    ...
    {
		throw new App_Storage_Shared_Adapter_ImageWorkshop_ImageWorkshopLayerException('Method cropInPourcent() was renamed cropInPercent(). Use cropInPercent() instead.', static::METHOD_DEPRECATED);
	}
    ...
    {
        throw new App_Storage_Shared_Adapter_ImageWorkshop_ImageWorkshopLayerException('Method cropMaximumInPourcent() was renamed cropMaximumInPercent(). Use cropMaximumInPercent() instead.', static::METHOD_DEPRECATED);
    }
}